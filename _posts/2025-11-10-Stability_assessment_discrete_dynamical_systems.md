...
* If $|f'(x_0)| < 1$: then $[f'(x_0)]^n$ approaches 0 as $n$ increases, perturbations decay → fixed point is **stable**;

* If $|f'(x_0)| > 1$: then $[f'(x_0)]^n$ approaches infinity as $n$ increases, perturbations amplify → fixed point is **unstable**;
...
| Fixed Point (1-periodic orbit) | Discrete System (map $x_{n+1}=f(x_n)$) | Derivative of map at fixed point $f'(x_0)$ (i.e., 1×1 Jacobian): <br><br>- If $\|f'(x_0)\| < 1$ → stable; <br>- If $\|f'(x_0)\| > 1$ → unstable; <br>- If $\|f'(x_0)\| = 1$ → non-hyperbolic (bifurcation critical point, e.g., flip bifurcation). |
