...
Under this premise, $|f'(x_0)| < 1$ corresponds to stability and $|f'(x_0)| > 1$ corresponds to instability. This conclusion is absolutely correct and serves as one of the core criteria for bifurcations in discrete systems (such as flip bifurcations).
...
|----------------|-------------|---------------------|
| Equilibrium Point | Continuous System (ODE $\dot{x}=f(x)$ ) | **Eigenvalues** of Jacobian matrix $J(x_0) = \frac{\partial f}{\partial x}\bigg\|_{x_0}$ : <br><br>- All eigenvalues have real parts < 0 → stable; <br><br>- At least one eigenvalue has real part > 0 → unstable; <br><br>- Some eigenvalues have real part = 0 → non-hyperbolic (bifurcation critical point, e.g., Hopf bifurcation). |
| Fixed Point (1-periodic orbit) | Discrete System (map $x_{n+1}=f(x_n)$ ) | Derivative of map at fixed point $f'(x_0)$ (i.e., 1×1 Jacobian): <br><br>- $\|f'(x_0)\| < 1$ → stable; <br><br>- $\|f'(x_0)\| > 1$ → unstable; <br><br>- $\|f'(x_0)\| = 1$ → non-hyperbolic (bifurcation critical point, e.g., flip bifurcation). |
| n-periodic orbit (n≥2) | Discrete System | Derivative of n-times iterated map $f^n(x) = f(f(\dots f(x)\dots))$ (n-fold composition) at periodic point: <br><br>- $\|(f^n)'(x_0)\| < 1$ → stable; <br><br>- $\|(f^n)'(x_0)\| > 1$ → unstable. <br><br>(Example: 2-periodic orbit requires computing $(f \circ f)'(x_0) = f'(x_1) \cdot f'(x_0)$ , where $x_0,x_1$ are 2-periodic points) |
| Periodic Orbit (Limit Cycle) | Continuous System | **Floquet multipliers** of the periodic orbit (describing perturbation evolution near the orbit): <br><br>- All Floquet multipliers have modulus < 1 (or real part < 0) → stable; <br><br>- At least one Floquet multiplier has modulus > 1 (or real part > 0) → unstable; <br><br>- Some multipliers have modulus = 1 → non-hyperbolic (bifurcation critical point). |
| Chaotic Attractor | Continuous/Discrete System | Examine **Lyapunov exponents**: <br><br>- At least one positive Lyapunov exponent → chaotic (perturbations grow exponentially, sensitive dependence on initial conditions); <br><br>- All Lyapunov exponents ≤ 0 → non-chaotic (stable periodic or steady state). |
